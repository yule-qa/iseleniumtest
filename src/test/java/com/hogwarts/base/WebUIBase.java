...
public abstract class WebUIBase {
    ...
    private Properties loadFromEnvProperties(String propFileName) {
        ...
        String path = System.getProperty("user.home"); //获取用户主目录路径 ，我的电脑就是/Users/christbella

        //读入envProperties属性文件
        try {
            prop = new Properties();
            InputStream in = new BufferedInputStream(
                    new FileInputStream(path + File.separator + propFileName));  //File.separator这个代表系统目录中的间隔符，说白了就是斜线，不过有时候需要双线，有时候是单线，你用这个静态变量就解决兼容问题了。
            prop.load(in);
            in.close();
    ...
    private void setCurBrowser() {
        String value = System.getenv("currentBrowser");  //从当前的环境变量中读取currentBrowser的值
        if (value == null || value.equalsIgnoreCase("")) {  //equalsIgnoreCase() 方法用于将字符串与指定的对象比较，不考虑大小写。
            return;
        }
